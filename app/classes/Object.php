...
class Objects {
	...
	public function create($table, $fields = array()) {
		// Check if 'last_update_at' is present in $fields array
		if (!isset($fields['last_update_at'])) {
			// If not, provide a default value (current date and time)
			$fields['last_update_at'] = date('Y-m-d H:i:s');
		}
...
$stmt->bindParam(":quantity", $remain_quantity, PDO::PARAM_INT);
$stmt->bindParam(":id", $pid[$i], PDO::PARAM_INT);
$stmt->execute();
				}
